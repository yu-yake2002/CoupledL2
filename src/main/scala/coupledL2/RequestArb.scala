...
class RequestArb(implicit p: Parameters) extends L2Module {
  ...
  /* status of each pipeline stage */
  io.status_s1.sets := VecInit(Seq(C_task.set, B_task.set, A_task.set, mshr_task_s1.bits.set))
  io.status_s1.tags := VecInit(Seq(C_task.tag, B_task.tag, A_task.tag, mshr_task_s1.bits.tag))
